...
/**
 * @todo: I should look into another system because of all the intern
 * fragmentation this system will cause
 */

/**
 * @optimize: 	- I don't think the used list should even exist lol
 * 				- The used list should not be sorted like it is right now
 *     			- If we remove free_node's used check we could speed up freeing
 * memory
 */
...
void print_list(struct ll_node *node)
{
	...
	do
	{
		debug_print_hex((int) node);
		debug_print(" - ");
		debug_print_hex((int) node + node->size);
		debug_print(" | ");
...
void add_to_free(struct ll_node *node, struct linkedlist *list)
{
	...
	/**
	 * I know this is a very weird loop but it's late and this works, i just
	 * wrote it weird.
	 *
...
static inline struct ll_node *expand_heap_align_split(size_t   nodesize,
                                                      offset_t heapend,
                                                      offset_t alignoffset,
                                                      struct linkedlist *list)
{
	...
	struct ll_node *newnode =
		(struct ll_node *) (heapend + alignoffset - sizeof(struct ll_node));
...
struct ll_node *expand_heap(size_t nodesize, struct linkedlist *list,
                            int pagealign)
{
	if (pagealign)
	{
		...
		if (offset + nodesize + list->cursize <= list->maxsize)
		{
			if (offset >= sizeof(struct ll_node) * 2 + NODESIZE_MAX)
			{
				...
				return expand_heap_align_split(
					nodesize, list->memstart + list->cursize, offset, list);
			}
			else if (offset == sizeof(struct ll_node))
			{
				...
				struct ll_node *newnode =
					(struct ll_node *) (list->memstart + list->cursize);
				newnode->size = nodesize;
				newnode->prev = newnode->next = 0;
			...
			else
			{
				 ...
				 */
				return expand_heap_align_split(nodesize,
				                               list->memstart + list->cursize,
				                               offset + 0x1000, list);
			}
		}
	}
...
struct ll_node *split_fit(size_t nodesize, struct ll_node *node,
                          struct linkedlist *list)
{
	/* If we can't create a whole new block out of it, just let the block be
	 * larger */
	...
	{
		struct ll_node *newnode = (struct ll_node *) ((offset_t) node + nodesize
	...
	if (pagealigned)
	{
		do
		{
			if (tmp->size >= searchsize)
			{
				...
				/* @todo       I actually turned this system off by returning
				 * null in check_pa_addr_in because i think this is just tooo
				 * buggy */
				 ...
				 */
				offset_t addr =
					check_pa_addr_in(tmp, searchsize + sizeof(struct ll_node));
				if (addr)
				{
					/* First cut off until new addr */
					size_t prevsize = tmp->size;
					tmp->size       = (addr - sizeof(struct ll_node))
								- ((offset_t) tmp + sizeof(struct ll_node));
					...
					if (((offset_t) tmp + sizeof(struct ll_node) + prevsize)
					        - (addr + searchsize)
					    > NODESIZE_MAX + sizeof(struct ll_node))
					{
						/* Append another block at the end */
						struct ll_node *endnode =
							(struct ll_node *) (addr + searchsize);
						endnode->size =
							(offset_t) tmp + sizeof(struct ll_node) + prevsize
							- (addr + searchsize + sizeof(struct ll_node));

						add_to_free(endnode, list);
					}
					else
						newnode->size =
							prevsize - sizeof(struct ll_node) - tmp->size;
...
{
	/**
	 * The only reason we keep the start used list and check if the node is in
	 * that list here is because of safety, we could remove these checks and
	 * speed up this whole memory manager but it would make the heap so much
	 * less safe and prone to corruptions.
	 */
