...
void error_message(offset_t addr, registers_t *regs, int present, int rw, int us, int reserved)
{
	...
	else 
	{
		...
		printk("\nframe: %x\n", get_page(addr, 0, get_current_dir()));
		printk_hd(get_page(addr, 0, get_current_dir()), 200);
		debug_handler(regs);
		send_sig(SIGKILL);
		for(;;);
	}
...
void page_fault(registers_t *regs)
{
	...
	int not_present = !(regs->err_code & 0x1);	/* Wheter the page was present */
	...
	if (not_present)
	{
		 ...
		 * with the program break and zero it out */
			...
			zero_page(faulting_address, get_current_task()->directory, get_current_task()->ring);
