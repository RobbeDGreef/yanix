/* @todo: Somehow sublime has removed the whole VGA system don't know why but it means i gotta rewrite this */
...
/**
 * @brief      This function will turn the regular vga colors to well... vga colors
 *
...
void vga_draw_char(char character, int x, int y, int frgcolor, int bgcolor)
{
	...
	vga_info.fb[(y * vga_info.width + x) * vga_info.bpc + 1] = (char) (bgcolor << 8) | (frgcolor & 0xF);
}
...
void vga_clear_screen()
{
	...
	{
		/* @todo: VGA DRIVER: im not sure if the background has te be reset to 0 or not */
...
void hook_vga_to_video(video_driver_t *driver)
{
	...
	driver->video_card_name	  = VGA_CARD_NAME;		/* This should actually be done by the PCI driver */
}
