...
/**
 * This array is just a way to convert the known vga terminal colors to full vesa 16mil colors
 */
 ...
 */
inline static void vesa_draw_pixel_inline(char *videomem, unsigned int offset, int color)
{
...
void init_vesa(void *physicallfb, unsigned int width, unsigned int height, unsigned int bpp)
{
	...
	//identity_map_memory_block((unsigned int) physicallfb, (unsigned int)physicallfb + buffer_size, 0, 1, g_kernel_directory);
}
...
/**
 * @brief      Returns the appropriate rgb colorcode for a corresponding vga color
 *
 * @param[in]  color  The color
 *
...
void vesa_draw_char(char character, int x, int y, int frgcolor, int bgcolor)
{
	...
	/**
	 * @todo: This font/drawing system is realllyyy bad and we need to upgragetbitde it 
