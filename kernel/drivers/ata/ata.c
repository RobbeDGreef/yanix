...
char *ata_error_to_string(int error)
{
	...
	case ATA_ERROR_TK0NF:
		return "Track 0 not found";
	case ATA_ERROR_AMNF:
		return "No address mark";
	default:
		return "Unknown";
	}
}
...
static void ata_select_drive(ata_drive_t *drive, int mode)
{
	...
	if (drive != selected_drive && mode != selected_mode)
	{
		...
		{
			port_byte_out(drive->base + ATA_REG_HDDEVSEL,
			              0xE0 | (drive->slave << 4) | (mode & 0xF));
			// port_byte_out(drive->base + ATA_REG_HDDEVSEL, 0xE0 |
			// (drive->slave << 4));
		}
		else
		{
...
/* inline */ static void ata_send_sec_and_lba(ata_drive_t * drive,
                                              unsigned long lba,
                                              unsigned long count)
{
#if 0
	...
	if (drive->mode == ATA_MODE_LBA48)
	{
		/* Send sector count */
		printk(KERN_WARNING
		       "Oh oh looks like you found a part of code that doesn't work "
		       "due to a bug. (ata.c) ata_send_sec_and_lba\n");
		port_byte_out(drive->base + ATA_REG_SECCOUNT1, count & 0xFF00);
		/* Send LBA in pieces */
		// port_short_out(drive->base + ATA_REG_LBA0, (uint16_t) ((lba >> (8 *
		// 3)) & 0xFF));
		...
		/* NOTE: these are commented out because LBA couldn't be a uint64_t
		 * because of compiler bug and all */
		// port_byte_out(drive->base + ATA_REG_LBA1, (uint8_t) ((lba >> (8 * 4))
		// & 0xFF)); port_byte_out(drive->base + ATA_REG_LBA2, (uint8_t) ((lba
		// >> (8 * 5)) & 0xFF));
	}
	/* Now send the first part of the LBA and sector count */
	port_byte_out(drive->base + ATA_REG_SECCOUNT0, count & 0xFF);
 ...
 * @return     error code
 */
ssize_t _atapio_read(ata_drive_t *drive, unsigned long lba, uint16_t *buffer,
                     size_t count)
{
	/*@BUG: Oke so due to a compiler bug the lba variable above can't be
	 * uint64_t (and thus the note below) */
...
retry:
	...
	// printk("END Bsy bit is %i\n", port_byte_in(drive->base+ATA_REG_STATUS) &
	// ATA_STATUS_BSY); printk("BSY BIT: %i\n", port_byte_in(0x1f0+7) & 0x80);

	return 0;
}

ssize_t _atapio_write(ata_drive_t *drive, unsigned long lba, const void *buffer,
                      size_t count)
{
	/* Select drive */
...
int init_ata(pci_device_t *pci_dev)
{
	...
	for (size_t channel = 0; channel < 2; channel++)
	{
		...
		/* If the channels were not specifically specified set them here to
		 * default */
		if (base == 0 || base == 1)
		{
		...
		for (size_t ms = 0; ms < 2; ms++)
		{
			...
			/**
			 * @NOTE: if we ever support anything else then PATA keep in mind
			 * that this ata cmd can be different for example PATAPI and SATAPI
			 * use ATA_CMD_INDENTIFY_EXT
			 */
			...
			/* At this point we can create a drive structure because we are sure
			 * it exists */
			...
			ata_info->signature =
				*(uint16_t *) (ident_buffer + ATA_IDENT_DEVICETYPE);
			ata_info->capabilities =
				*(uint16_t *) (ident_buffer + ATA_IDENT_CAPABILITIES);
			...
			else
			{
				...
				ata_info->max_lba =
					*(uint32_t *) (ident_buffer + ATA_IDENT_MAX_LBA);
			}
